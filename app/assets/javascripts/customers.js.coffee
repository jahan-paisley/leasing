...
$ ->
  date_bind = $(".date").each (index, element) =>
    id = $(element).attr("id")
    id_btn = id + "btn"
    v = element.value
    element.value = JalaliDate.gregorianToJalali(v.substr(0, 4), v.substr(5, 2), v.substr(7, 2)).join('\/') if v && v[0..1] != "13"
    Calendar.setup({
    inputField: id,
    button: id_btn,
    ifFormat: '%Y/%m/%d',
...
#  $("div[id*=list]").bind 'cocoon:after-insert', (e, inserted_item) =>
#    date_bind
